...
const PhoneFrame: React.FC<PhoneFrameProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-start p-2 sm:p-6 font-sans">
      ...
      <header className="w-full max-w-xl mb-4 bg-slate-800/90 backdrop-blur-md rounded-2xl p-3 border border-slate-700/80 shadow-lg flex flex-wrap items-center justify-between gap-2 text-xs">
      ...
      {/* Demo Controls Bar */}
      {showDemoControls && (
        <section className="w-full max-w-xl mb-4 bg-blue-950/40 border border-blue-800/60 rounded-xl p-3 text-xs">
      ...
      {/* Main iPhone Frame Viewport (~390px fixed width) */}
      <main className="relative w-full max-w-[390px] h-[844px] bg-white text-slate-800 rounded-[48px] shadow-2xl border-[10px] border-slate-800 overflow-hidden flex flex-col select-none ring-1 ring-slate-700/50">
        
        {/* iOS Dynamic Island & Status Bar */}
        <div className="relative bg-white z-40 px-6 pt-3 pb-2 flex items-center justify-between text-slate-900 font-semibold text-xs border-b border-slate-100">
        ...
        <div className="w-full bg-white py-2 flex justify-center items-center z-40 border-t border-slate-100">
